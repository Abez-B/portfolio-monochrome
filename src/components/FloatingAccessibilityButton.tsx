...
export const FloatingAccessibilityButton: React.FC = () => {
  ...
  // Map icons dynamically to each page target
  ...
  // Arc parameters for wide, non-overlapping radial spread (from -180 deg to -10 deg)
  const startAngle = -180 * (Math.PI / 180);
  const endAngle = -10 * (Math.PI / 180);
  const radius = 135; // 135px radius provides ample space to prevent overlap
  ...
  return (
    <motion.div
      ...
      initial={{ x: 0, y: 0 }}
      className="fixed bottom-7 right-7 z-[80] select-none touch-none"
    >
